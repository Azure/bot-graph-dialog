...
class GraphDialog {
  ...
  async init() {
    ...
    this.proxyMode = this.options.proxyMode;
    if (!this.nav) {
      try {
  ...
  async stepInteractionHandler(session, results, next) {
    ...
    switch (currentNode.type) {
      ...
      case Node.NodeType.text:
        session.send(this.generateTextMessage(session, currentNode));
        return next();
      ...
      case Node.NodeType.carousel:
        session.send(this.generateCarouselMessage(session, currentNode, builder.AttachmentLayout.carousel));
        return next();

      case Node.NodeType.list:
        session.send(this.generateCarouselMessage(session, currentNode, builder.AttachmentLayout.list));
        return next();
      
      case Node.NodeType.adaptive:
        session.send(this.generateAdaptiveCardMessage(session, currentNode));
        return next();
  ...
  // generate text message with suggested actions
  generateTextMessage(session, node) {

    var text = strformat(node.data.text, session.dialogData.data);
    var msg = new builder.Message(session).text(text);

    if (node.data.buttons) {
      var buttons = this.addButtonsByType(session, node.data.buttons);
      if (buttons.length > 0) {
    ...
    var spoken = node.data.speak || text;
    msg.speak(this.speak(session, spoken))
    if (node.data.inputHint) {
      msg=this.addInputHint(node.data.inputHint,msg)
    }


    console.log("text message node " + node.id + ", msg: '" + msg + "'");
    return msg;
  }
  ...
  generateHeroCardMessage(session, node) {
    ...
    var msg = new builder.Message(session)
      ...
      .attachments([hero]);
    if (node.data && node.data.speak) {
      msg.speak(this.speak(session, node.data.speak));
    }
    if (node.data && node.data.inputHint) {
      msg=this.addInputHint(node.data.inputHint,msg)
    }
    return msg;
  };
  ...
  generateCarouselMessage(session, node, attachmentLayout) {
    //var _this = this;
    var data = node.data;
    ...
    var msg = new builder.Message(session)
      ...
      .attachments(cards);
    if (data.speak) {
      msg.speak(this.speak(session, data.speak));
    }
    if (data.inputHint) {
      switch (data.inputHint) {
        case builder.InputHint.acceptingInput:
        case builder.InputHint.expectingInput:
        case builder.InputHint.ignoreInput:
          msg.inputHint(data.inputHint);
      }
    }
  ...
  generateAdaptiveCardMessage(session, node) {
    if (node.data.text) {
      var text = strformat(data.text, session.dialogData.data);
      session.send(text);
    }
  ...
  addButtonsByType(session, inputButtons) {
    var buttons = [];
    for (let item of inputButtons) {
        switch (item.action) {
          case "openUrl":
          ...
          case "imBack":
            ...
            break;
          case "postBack":
            buttons.push(builder.CardAction.postBack(session, item.value, item.label));
            break;
        }
      }
      return buttons;
  ...
  speak(session, prompt) {
    var localized = session.gettext(prompt) || prompt;
    return ssml.speak(localized);
  }

  addInputHint(inputHint, msg) {
    switch (node.data.inputHint) {
      case builder.InputHint.acceptingInput:
      case builder.InputHint.expectingInput:
      case builder.InputHint.ignoreInput:
        msg.inputHint(node.data.inputHint);
    }
    return msg;
  }
